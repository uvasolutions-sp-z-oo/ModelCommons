...
async function operation<T>(name: string, path = "", value = ""): Promise<T> {
  ...
  const method = native?.privateModelOperation;

  if (!native || typeof method !== "function") {
    throw new ModelCommonsError(
      "RUNTIME_UNAVAILABLE",
      "Private model storage requires a new native build.",
    );
  }

  return callNative(
    () => method.call(native, name, path, value) as Promise<T>,
    {
      fallbackCode: "STORAGE_UNAVAILABLE",
      fallbackMessage: "Private model storage operation failed.",
    },
  );
}
...
export async function createPrivateStorePort(): Promise<WriteStorePort> {
  ...
  return {
    ...
    atomicText: (path, value) =>
      operation("write", assertSafeRelativePath(path), value),
    ...
    async download(url, path, options) {
      const native = nativeModule;
      const downloadPrivateModel = native?.downloadPrivateModel;

      if (!native || typeof downloadPrivateModel !== "function") {
        throw new ModelCommonsError(
          "RUNTIME_UNAVAILABLE",
          "Native model provisioning is unavailable.",
        );
      }
      ...
      const job = callNative(
        () =>
          downloadPrivateModel.call(
            native,
            id,
            url,
        ...
        {
          fallbackCode: "STORAGE_UNAVAILABLE",
          fallbackMessage: "Model provisioning failed.",
        },
      );
      const abort = () => {
        void operation("cancel", id).catch(() => undefined);
      };
      options.signal?.addEventListener("abort", abort, { once: true });
      const poll = setInterval(() => {
        if (options.signal?.aborted) abort();
        void operation<number>("progress", id)
          .then((written) => {
    ...
    async importFile(source, path, signal, expectedBytes) {
      ...
      if (
        ...
        throw new ModelCommonsError(
          "PERMISSION_REQUIRED",
          "Choose an approved GGUF using the private import picker.",
        );
      }
      const native = nativeModule;
      const importPrivateModel = native?.importPrivateModel;

      if (!native || typeof importPrivateModel !== "function") {
        throw new ModelCommonsError(
          "RUNTIME_UNAVAILABLE",
          "Private import requires a new native build.",
        );
      }
      if (signal?.aborted)
        throw new ModelCommonsError("USER_CANCELLED", "Import cancelled.");
      ...
      const job = callNative(
        ...
        {
          fallbackCode: "STORAGE_UNAVAILABLE",
          fallbackMessage: "Private import failed.",
        },
      );
